...
from gmail_fast_mcp import filter_templates
...
def create_filter(
    criteria: Annotated[
        dict,
        "Criteria for matching emails (from, to, subject, query, negatedQuery, hasAttachment, excludeChats, size, sizeComparison)",
    ],
    action: Annotated[
        dict, "Actions to perform (addLabelIds, removeLabelIds, forward)"
    ],
) -> str:
...
def list_filters() -> str:
    ...
    gmail = get_gmail_service()
    resp = gmail.users().settings().filters().list(userId="me").execute()
...
def get_filter(
    ...
    gmail = get_gmail_service()
    result = gmail.users().settings().filters().get(userId="me", id=filter_id).execute()
    return (
        f"Filter details:\n"
        f"ID: {result.get('id')}\n"
...
def create_filter_from_template(
    template: Annotated[
        str,
        "Template name: fromSender, withSubject, withAttachments, largeEmails, containingText, or mailingList",
    ],
    ...
    search_text: Annotated[
        str | None, "Text to search for (for containingText)"
    ] = None,
    list_identifier: Annotated[
        str | None, "Mailing list identifier (for mailingList)"
    ] = None,
    size_in_bytes: Annotated[
        int | None, "Size threshold in bytes (for largeEmails)"
    ] = None,
    ...
    if template == "fromSender":
        ...
        config = filter_templates.from_sender(sender_email, label_ids, archive or False)
    elif template == "withSubject":
        if not subject_text:
            raise ValueError("subject_text is required for withSubject template")
        config = filter_templates.with_subject(
            subject_text, label_ids, mark_as_read or False
        )
    elif template == "withAttachments":
        config = filter_templates.with_attachments(label_ids)
    elif template == "largeEmails":
        if not size_in_bytes:
            raise ValueError("size_in_bytes is required for largeEmails template")
        config = filter_templates.large_emails(size_in_bytes, label_ids)
    elif template == "containingText":
        if not search_text:
            raise ValueError("search_text is required for containingText template")
        config = filter_templates.containing_text(
            search_text, label_ids, mark_important or False
        )
    elif template == "mailingList":
        if not list_identifier:
            raise ValueError("list_identifier is required for mailingList template")
        config = filter_templates.mailing_list(
            list_identifier, label_ids, archive if archive is not None else True
        )
    else:
        raise ValueError(
            f"Unknown template: {template}. "
            f"Valid: {', '.join(filter_templates.TEMPLATES)}"
        )

    gmail = get_gmail_service()
    result = (
        gmail.users().settings().filters().create(userId="me", body=config).execute()
    )
    return (
